...
public class Spy
{
    ...
    public string CollectGettersAndSetters(string investigatesClass)
    {
        var type = Type.GetType(investigatesClass);
        var sb = new StringBuilder();
        ...
        foreach (var property in properties)
        {
                ...
                sb.AppendLine
                    ($"{property.GetMethod.Name} will return {property.GetMethod.ReturnType}");
        }
        foreach (var property in properties)
        {
                if(property.SetMethod != null)
                sb.AppendLine
                    ($"{property.SetMethod.Name} will set field of {property.SetMethod.GetParameters().First().ParameterType}");
        }
        return sb.ToString().Trim();
    }
    //public string AnalyzeAcessModifiers(string classToInvestigate)
